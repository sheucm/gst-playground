...
int main(int argc, char *argv[]) {
  ...
  /* Create the elements */
  data.uridecodebin = gst_element_factory_make ("uridecodebin", NULL);
  ...
  if (!data.pipeline || !data.uridecodebin 
  	|| !data.videoconvert || !data.autovideosink
  	|| !data.audioconvert || !data.autoaudiosink) {
  ...
  gst_bin_add_many (GST_BIN (data.pipeline), data.uridecodebin, 
  	data.videoconvert, data.autovideosink,
  	data.audioconvert, data.autoaudiosink, NULL);
  if (!gst_element_link (data.videoconvert, data.autovideosink) ||
  	!gst_element_link (data.audioconvert, data.autoaudiosink)) {
  ...
  /* Set the URI to play */
  g_object_set (data.uridecodebin, "uri", "https://www.freedesktop.org/software/gstreamer-sdk/data/media/sintel_trailer-480p.webm", NULL);
